...
contract ve is IERC721, IERC721Metadata {
    ...
    uint256 internal constant MAXTIME = 4 * 365 * 86400; // max time = seconds in four years
    ...
    function _removeTokenFromOwnerList(address _from, uint256 _tokenId) internal {
        ...
        uint256 current_count = _balance(_from) - 1; // 3
        uint256 current_index = tokenToOwnerIndex[_tokenId]; // [0, 1, 2, 3]

        if (current_count == current_index) {
            // if current index is 3, in this case
            // update ownerToNFTokenIdList
            ownerToNFTokenIdList[_from][current_count] = 0; // sets 7 to 0
            // update tokenToOwnerIndex
            tokenToOwnerIndex[_tokenId] = 0; // sets 3 to 0
        } else {
            // if current index is [0, 1, 2]
            uint256 lastTokenId = ownerToNFTokenIdList[_from][current_count]; // 7
    ...
    function totalSupplyAt(uint256 _block) external view returns (uint256) {
        ...
        if (target_epoch < _epoch) {
            // otherwise dt = 0 and we can use the point at target to get supply
            ...
            if (point.blk != point_next.blk) {
                // otherwise dt = 0 as the two boundary blocks are the same block
            ...
            if (point.blk != block.number) {
                // otherwise dt = 0 as the lower block and current block are the same block

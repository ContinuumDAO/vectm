...
contract DelDAppAddr is Script {
    ...
    function run() public {
        ...
        try vm.envAddress("CTM_TO_REMOVE") returns (address _ctm) {
            ctm = _ctm;
        } catch {
            revert("CTM_TO_REMOVE not defined");
        }

        vm.startBroadcast(deployer);

        console.log("Removing CTM address in DApp Manager...");

        IC3DAppManager(dappManagerAddr).setDAppAddr(dappID, ctm, false);

        console.log("CTM address removed.");

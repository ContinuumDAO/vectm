...
contract DeployCtmMintable is Script {
    ...
    function _setPeersForAllChains(string memory toml, CTMMintable ctmMintable) internal {
        string[] memory chainKeys = vm.parseTomlKeys(toml, ".");
        string memory peerAddr = vm.toString(address(ctmMintable));
        for (uint256 i = 0; i < chainKeys.length; i++) {
            ...
            if (_isNumericChainId(chainId)) {
                ...
                ctmMintable.setPeer(chainId, peerAddr);
            }
        }
    }

...
import { IERC6372, IGovernor } from "@openzeppelin/contracts/governance/IGovernor.sol";
...
abstract contract Governor is Context, ERC165, EIP712, Nonces, IGovernor, IERC721Receiver, IERC1155Receiver {
    ...
    * that no other address can submit the same proposal. An attacker would have to either remove or change that part,
    ...
    // WARN: The following five functions are added only to allow access to private state in `Governor`.
    // Alternatively, the visibility of the referenced state could be set to `internal`.

    // INFO: This function is here to allow override of `execute`, because `_proposals` is private.
    function _setProposalExecuted(uint256 proposalId) internal {
        _proposals[proposalId].executed = true;
    }

    // INFO: This function is here to allow override of `execute`, because `_governanceCall` is private.
    ...
    // INFO: This function is here to allow override of `queue`, because `_proposals` is private.

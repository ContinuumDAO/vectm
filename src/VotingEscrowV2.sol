...
contract VotingEscrowV2 is UUPSUpgradeable, IERC721Metadata, IVotingEscrow, IVotes {
    ...
    uint256 public constant LIQUIDATION_PENALTY = 50000; // 50000 / 100000 = 50 %
    ...
    function create_lock_for_non_voting(uint256 _value, uint256 _lock_duration, address _to)
        external
        nonreentrant
        returns (uint256)
    {
    ...
    function split(uint256 _tokenId, int128 _extracted) external checkNotAttached(_tokenId) returns (uint256) {
        ...
        require(_extracted < value);
        int128 remainder = value - _extracted;
        locked[_tokenId] = LockedBalance(remainder, _locked.end);
        _checkpoint(_tokenId, _locked, LockedBalance(remainder, _locked.end));
        uint256 extracted = uint256(int256(_extracted));

        if (nonVoting[_tokenId]) {
            uint256 extractedTokenId = _create_lock(extracted, _locked.end, msg.sender);
            nonVoting[extractedTokenId] = true;
            return extractedTokenId;
        } else {
            return _create_lock(extracted, _locked.end, msg.sender);
        }
    }
    ...
    function _add(uint256[] memory current, uint256[] memory addIDs) internal pure returns (uint256[] memory) {
        uint256 updatedLength = current.length + addIDs.length;
        uint256[] memory updated = new uint256[](updatedLength);
        for (uint256 i = 0; i < current.length; i++) {
            updated[i] = current[i];
        }
        for (uint256 i = current.length; i < current.length + addIDs.length; i++) {
            updated[current.length + i] = addIDs[i];
        }
        return updated;
    }

    function _remove(uint256[] memory current, uint256[] memory removeIDs) internal pure returns (uint256[] memory) {
        uint256[] memory updated = new uint256[](current.length - removeIDs.length);
